...
public class OperandFetch {
	...
	public void performOF() {
		...
		if(containingProcessor.getEXUnit().EX_MA_Latch.getInstruction()!=0){
			...
			//System.out.println("INSTRUCTION MA"+containingProcessor.getEXUnit().EX_MA_Latch.getInstruction());
		...
		if (counter == 1) {
			...
			if (opcode >= 0 && opcode < 22) {
				...
				if (opcode % 2 == 0) {
					...
					if (((addOP1 == addEX )|| (addOP1 == addMA)|| (addOP1 == addRW)|| (addOP2 == addEX)|| (addOP2 == addMA) || (addOP2 == addRW)) && (addEX!=100 || addMA!=100  || addRW!=100)) {
					...
					if ((addOP1 == addEX && addEX!=100)|| (addOP1 == addMA && addMA!=100)|| (addOP1 == addRW && addRW!=100)|| (addOP2 == addEX && addEX!=100)|| (addOP2 == addMA && addMA!=100)
							|| (addOP2 == addRW&& addRW!=100) || addOP1==31 || addOP2==31) {
						IF_EnableLatchType.setIF_enable(false);
						IF_OF_LatchType.setOF_enable(false);
			...
			//System.out.println("Instruction in MALATCH " + containingProcessor.getEXUnit().EX_MA_Latch.getInstruction());
			//System.out.println("Instruction in RWLATCH" + containingProcessor.getMAUnit().MA_RW_Latch.getInstruction());

			if ( OF_EX_Latch.getInstruction()==containingProcessor.getEXUnit().EX_MA_Latch.getInstruction() && containingProcessor.getEXUnit().EX_MA_Latch.getInstruction()==containingProcessor.getMAUnit().MA_RW_Latch.getInstruction()&&
